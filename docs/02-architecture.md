...
2. 点「开始游戏」 → import('/assets/games/2048.js')   ← 这时才下载游戏代码
3. POST /api/matches { gameId } → 返回 { matchId, seed, ticket }
4. 游戏全程在浏览器里跑，每步只记录到内存数组，不发请求 ← 零延迟、零服务端压力
5. 终局 → POST /api/matches/{id}/finish { ticket, moves[], stepMs[], durationMs, finalHash }
6. 服务端用同一份内核 replay 一遍 moves：
     - 校验每步合法、终局哈希一致
     - 反作弊检查（见下）
     - 写 R2 moves.jsonl + 回放包
     - D1 写 1 行 matches + 1 行 match_players + 排行榜 upsert
```
...
第 3 步**不写数据库**。`ticket` 是一个签名的 JWT，里面装着
`{ matchId, gameId, seed, startedAt }`，有效期 12 小时。终局时客户端把它交回来，
服务端验签之后就能信任这几个字段。

这样一来：

- 开局零写入，**半途放弃的对局在 D1 里不留任何痕迹**（这类对局占大多数）
- 不存在「进行中」的行，`matches` 表里只有真正打完的局
- 客户端伪造不了 seed 和开局时间，因为改一个字节签名就对不上
- 重复提交同一个 matchId 由主键 `INSERT OR IGNORE` 挡住，天然幂等

服务端 replay 的成本极低：2048 跑 800 步纯数组运算不到 1ms。

### 信息不完全的游戏：无状态逐步提交

扫雷和记忆翻牌的隐藏层必须由服务端持有，客户端算不出下一帧，
所以它们改为每步提交一次 `POST /api/matches/{id}/step { ticket, moves[] }`。

关键在于服务端**不保存中间状态**：每次收到完整的走法序列，从 seed 重放一遍，
把可见视图算出来返回。80 步的一局重放开销是微秒级。

因此这条路径同样没有 Durable Object、没有会话、没有数据库写入——
只是把「一次提交全部」换成了「每步提交一次」。
...
- **seed 由服务端签在票据里**，客户端拿不到"未来"的随机数，也改不了开局时间
- **服务端 replay 全部走法**，非法走法直接拒绝——伪造必须是"真的算出一个合法高分序列"
- **时间校验**：客户端上报的 `durationMs` 会被夹在「票据签发到现在的真实经过时间 + 5 秒」以内，
  再要求 `durationMs >= 步数 × 每步最小耗时`。这条同时挡住了「秒交」和「谎报用时」两种玩法
- **节奏异常检测**：逐步时间戳的变异系数过小（机器般均匀）判为可疑。
  贪吃蛇这类本来就匀速自动推进的游戏按 `ui.autoTickMs` 跳过这一项，避免误伤
- 判为可疑的对局**照常记录和回放，但状态标记为 `flagged` 且不进排行榜**——
  不直接拒绝，是为了避免误伤真实玩家时把成绩弄丢
- **双人游戏的人类赛道不计评分**：一个人自己两边下只能算练习，
  记录对局和回放，但不产生 Elo。双人排名来自 AI 对打或将来的真人对战
