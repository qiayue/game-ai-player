...
## 问题 11：读快照时把刚更新过的榜当成了新鲜数据

**症状**：终局时只失效了 KV 缓存，但 D1 里的 `leaderboard_snapshot` 还在
「10 分钟内算新鲜」的窗口里。于是下一次读排行榜会跳过重建，直接返回旧快照——
玩家刚打完一局，榜上却看不到自己。

**对策**：终局时除了删 KV 缓存，再写一个 `dirty:lb:{game}:{track}:{window}` 标记。
读路径只在**缓存未命中时**多读一次这个标记：有标记就必须重建，没标记才敢用快照。
热路径依然是一次 KV 读，代价只落在缓存刚失效的那一次请求上。
Cron 也用这些标记来决定重建哪些榜——而不是遍历「所有游戏 × 赛道 × 时间窗」那 66 个组合。

> 这个 bug 是端到端冒烟测出来的，单测发现不了：它需要「先读一次空榜、再写入、再读」这个顺序。

---

## 问题 12：异步落库期间页面 404

**症状**：AI 跑完后前端立刻拿到 matchId，但 D1 的写入要经过 Queue 批量合并，
最长要等一个 `max_batch_timeout`。这段时间里回放页查 D1 查不到，直接 404。

**对策**：回放页在 D1 没有这一行时**回落到 R2 的回放包**渲染
（R2 是终局时同步写的，一定已经就绪），页面正常显示并提示「正在结算」，
同时标记 `noindex` 且只缓存几秒。批量合并的延迟因此对用户完全不可见。

顺带把 `max_batch_timeout` 从 10 秒降到 5 秒，缩短窗口。
...
- [ ] 开局不写库（用签名票据），只有打完的对局才在 D1 里有行
- [ ] 排行榜写入时打「脏」标记，读路径据此决定是否重建快照
- [ ] 异步落库期间的页面有 R2 兜底，不会 404
- [ ] 模型横评那类聚合查询只在 Cron 里跑并缓存，永远不在请求路径上
